...
class OpenCamera3 extends Component {
  constructor(props) {
    super(props);
    this.state = {
      imageData3: '',
  ...
  takePicture = async function() {
    ...
    if (this.camera) {
      try {
        ...
        this.setState({imageData3: source}, () => {
          this.props.saveImage3(this.state.imageData3);
        });
        // }
      } catch (e) {
        console.log(e);
  ...
  goBack() {
    ...
    this.setState(
      ...
      () => console.log('camera status on third', this.state.cameraStatus),
    );
    if (this.props.editval === true) {
      Actions.edit3();
    } else {
...
const mapStateToProps = state => {
  console.log('first button selected', state.login.thirdImage);
  console.log('cam status 3', state.login.camStatus3);
  return {
